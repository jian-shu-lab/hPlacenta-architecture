...
def _get_final_mapping_data(sample = 'JS40', n_neighbors = 100):
    ...
    adata_ref = sc.read('/home/jinmr2/sample_integration/three_samples/imputation/v6_map_ATAC/single-cell_atac_data_reference/filtered_genescore_bc_matrix.h5ad')
    adata_sc_imputation_X = adata_ref.X.toarray() # already normalized to 10000
    ...
    adata_imput_expr = AnnData(
        X = st_imputation,
        obs = adata.obs.copy(),
        var = pd.DataFrame(index = adata_ref.var.index.values)
    )
    adata_imput_expr.write(f'output/adata_imput_atac_geneactivity_{sample}_{n_neighbors}_final.h5ad')
